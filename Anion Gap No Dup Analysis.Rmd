...
Inpt.plot <- highRateComparison(
  ...
  highlevel = high) +
  labs(title = paste0("Proportion of Anion Gap ≥ ", high," by Hour Completed, Inpatient (weekdays)")) 

Emerg.plot <- highRateComparison(
  filter(df.2022.rmdup.wkday, Location == "Emergency"),
  filter(df.2023.rmdup.wkday, Location == "Emergency"),
  highlevel = high) +
  labs(title = paste0("Proportion of Anion Gap ≥ ", high," by Hour Completed, Emergency (weekdays)")) 
...
Outpt.plot <- highRateComparison(
  ...
  highlevel = high) +
  labs(title = paste0("Proportion of Anion Gap ≥ ", high," by Hour Completed, Outpatient (weekdays)")) 
...
Inpt.plot.InLab <- highRateComparison.InLab(
  ...
  labs(title = paste0("Proportion of Anion Gap ≥ ", high.InLab," by Hour Received in Lab, Inpatient (weekdays)")) 
...
Emerg.plot.InLab <- highRateComparison.InLab(
  ...
  highlevel = high.InLab) +
  labs(title = paste0("Proportion of Anion Gap ≥ ", high.InLab," by Hour Received in Lab, Emergency (weekdays)")) 
...
Outpt.plot.InLab <- highRateComparison.InLab(
  ...
  highlevel = high.InLab) +
  labs(title = paste0("Proportion of Anion Gap ≥ ", high.InLab," by Hour Received in Lab, Outpatient (weekdays)")) 
...
ggplot(df.2022.rmdup.wkday.tat, aes(x = TAT_hrs, group = Location)) +
  geom_density() +
  coord_cartesian(xlim = c(0, 12))

ggplot(df.2023.rmdup.wkday.tat, aes(x = TAT_hrs, group = Location)) +
  geom_density() +
  coord_cartesian(xlim = c(0, 12))
...
ggplot(df.2022.rmdup.wkday.tat, aes(x = Time)) +
  ...
  theme_minimal() +
  labs(title = "2022 Anion Gap Turnaround Time vs Hour Completed",
...
ggplot(df.2022.rmdup.wkday.tat, aes(x = Time.InLab, group = Time.InLab)) +
  ...
  scale_y_continuous(breaks = seq(0, 12, 2)) +
  theme_minimal() +
  labs(title = "2022 Anion Gap Turnaround Time vs Hour In Lab",
       subtitle = "105 points with TAT > 12 hrs not shown") +
  xlab("Hour In Lab") +
  ylab("Turnaround Time (hrs)")
...
ggplot(df.2023.rmdup.wkday.tat, aes(x = Time)) +
  ...
  scale_y_continuous(breaks = seq(0, 12, 2)) +
  theme_minimal() +
  labs(title = "2023 Anion Gap Turnaround Time vs Hour Completed",
...
ggplot(df.2023.rmdup.wkday.tat, aes(x = Time.InLab, group = Time.InLab)) +
  ...
  scale_y_continuous(breaks = seq(0, 12, 2)) +
  theme_minimal() +
  labs(title = "2023 Anion Gap Turnaround Time vs Hour In Lab",
       subtitle = "14 points with TAT > 12 hrs not shown") +
  xlab("Hour In Lab") +
  ylab("Turnaround Time (hrs)")
...
ggplot(df.2022.rmdup.wkday.tat, aes(x = TAT_hrs, y = RESULT, color = factor(Time.InLab))) +
  ...
  xlab("Turnaround Time (hours)")
# Looks like slightly skewed graph regardless of time
...
# Add specific labels to each of these graphs so that it's clear what they are
ggplot(df.2022.rmdup.wkday.tat, aes(x = TAT_hrs, y = RESULT)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "2022 Scatterplot of Anion Gap vs Turnaround Time")

ggplot(df.2023.rmdup.wkday.tat, aes(x = TAT_hrs, y = RESULT)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "2023 Scatterplot of Anion Gap vs Turnaround Time")
...
# There isn't an obvious relationship between TAT and AG. Linear regression is basically the same for both 2022 and 2023
# If you restrict regression to AG values of ≥16, then there IS a noticeable positive relationship 
  # between TAT and AG, which is larger in 2022 than 2023. 
  # Is the justification for this reasonable? We know that there is a higher rate of "elevated" AG values in 2022 than in 2023 we're specifically interested in these values, so that's why we want to do only a regression on those values
  # Would be helpful to have data from Eldad showing how AG changes when you leave blood sitting out in a cup - that would inform regression cut-offs based on TAT rather than AG (bc AG feels a little like backwards calculating)
